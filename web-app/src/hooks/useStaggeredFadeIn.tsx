...
export function StaggeredAnimationProvider({
  ...
  const setSidebarAnimated = useAnimationStore(
    (state) => state.setSidebarAnimated,
  );
  ...
  return (
    <StaggeredAnimationContext.Provider
      value={{ getIndex, delayMs, initialDelayMs, ready, skipAnimation }}
    >
      {children}
    </StaggeredAnimationContext.Provider>
  );
...
export function useStaggeredFadeIn(explicitIndex?: number) {
  const { getIndex, delayMs, initialDelayMs, ready, skipAnimation } =
    useContext(StaggeredAnimationContext);
