          ...
          <div className="shrink-0 px-10 w-[480px] mx-auto overflow-auto pb-10 pointer-events-auto -mt-20">
            <div className="mb-4">
              <h1 className="font-studio font-medium text-2xl mb-1">
                {isDownloading ?  'Sit tight, RongxinAI is getting ready...' : 'Hey, welcome to RongxinAI!'}
              </h1>
              <p className='text-muted-foreground leading-normal w-full mt-1'>{isDownloading ? 'This may take a few minutes.' : 'RongxinAI needs a model to begin. Let’s set it up.'}</p>
            </div>
                ...
                <div className={cn("bg-secondary/50 p-3 rounded-lg border transition-all hover:shadow disabled:opacity-60 flex justify-between items-start")}>
                  <div className="flex w-full items-start gap-4">
                    <div className="shrink-0 size-12 bg-background rounded-xl flex items-center justify-center">
                      <img src="/images/jan-logo.png" alt="RongxinAI Logo" className='size-6' />
                    </div>
                    <div className="flex flex-col w-full h-full justify-center">
                      <div className="flex flex-1 items-center justify-between">
                        <h1 className="font-semibold text-sm mb-1">
                          <span>{janNewModel?.display_name ?? janNewModel?.model_name ?? 'RongxinAI Model'}</span>&nbsp;<span className='text-xs text-muted-foreground'>· {defaultVariant?.file_size}</span>
                        </h1>
                        {(isDownloading) && (
