...
// Create a local-based conversation service instance
const service = conversationService('cloud');
...
export const useConversations = create<ConversationState>((set, get) => ({
  ...
  getUIMessages: async (conversationId: string, branch?: string) => {
    try {
      ...
      const items = (
        await service.getItems(conversationId, branchToUse)
      ).data;
  ...
  createItems: async (conversationId: string, items: CreateItemRequest[]) => {
    try {
      return await service.createItems(conversationId, items);
    } catch (err) {
  ...
  updateConversation: async (
    ...
    try {
      const updatedConversation = await service.updateConversation(
        conversationId,
        payload,
      );
      set((state) => ({
  ...
  fetchBranches: async (conversationId: string) => {
    try {
      ...
      const response = await service.getBranches(conversationId);
  ...
  switchBranch: async (conversationId: string, branchName: string) => {
    try {
      await service.activateBranch(conversationId, branchName);
  ...
  deleteBranch: async (conversationId: string, branchName: string) => {
    try {
      await service.deleteBranch(conversationId, branchName);
      set((state) => ({
  ...
  editMessage: async (
    ...
    try {
      const response = await service.editMessage(
        conversationId,
        itemId,
        content,
        regenerate,
  ...
  regenerateMessage: async (
    ...
    try {
      const response = await service.regenerateMessage(
        conversationId,
        itemId,
        options,
      );
  ...
  deleteMessage: async (conversationId: string, itemId: string) => {
    try {
      const response = await service.deleteMessage(
        conversationId,
        itemId,
      );
      return response;
