 ...
 * @returns
 */
export async function janProvider(
  selectedModel?: Model,
  conversationId?: string,
  deepResearch?: boolean,
  isPrivateChat?: boolean,
  isEnableThinking: boolean = false
): Promise<OpenAICompatibleProvider<string, string, string>> {
  ...
  if (selectedModel?.providerId) {
    ...
    await serviceHub
      ?.models()
      .startModel(providers?.find((e) => e.provider === 'llamacpp')!, modelId)

    // Import invoke dynamically to avoid issues when not in Tauri context
    ...
    const sessionInfo = await invoke<{
      ...
      api_key: string
      mmproj_path?: string
    } | null>('plugin:llamacpp|find_session_by_model', {
      modelId,
    })
