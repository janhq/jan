...
describe('ModelFactory', () => {
  ...
  describe('createModel', () => {
    ...
    it('should create an OpenAI-compatible model for novita provider', async () => {
      const provider: ProviderObject = {
        provider: 'novita',
        api_key: 'test-api-key',
      ...
      const model = await ModelFactory.createModel('meta-llama/llama-3.3-70b-instruct', provider)
      expect(model).toBeDefined()
      expect(model.type).toBe('openai-compatible')
    })
