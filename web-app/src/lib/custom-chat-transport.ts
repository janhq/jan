...
export class CustomChatTransport implements ChatTransport<UIMessage> {
  ...
  async sendMessages(
    ...
    // #region agent log
    const __dbgRunId = `tps-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`
    let __dbgFirstDeltaTs: number | undefined
    let __dbgPrevDeltaTs: number | undefined
    let __dbgDeltaCount = 0
    let __dbgDeltaCharSum = 0
    let __dbgInterDeltaSumMs = 0
    let __dbgInterDeltaMaxMs = 0
    const __dbgPost = (
      message: string,
      hypothesisId: string,
      ...
      try {
        void fetch(
          'http://127.0.0.1:7576/ingest/349dbbed-26a7-42bf-b66c-4a6027726691',
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'X-Debug-Session-Id': 'ae820a',
            },
            body: JSON.stringify({
              sessionId: 'ae820a',
              runId: __dbgRunId,
              hypothesisId,
              location: 'custom-chat-transport.ts:debugLogChunk',
              message,
              data,
              timestamp: Date.now(),
            }),
    ...
    const debugLogChunk = (
      ...
      if (part.type === 'text-delta' || part.type === 'reasoning-delta') {
        if (__dbgFirstDeltaTs === undefined) {
          __dbgFirstDeltaTs = now
          __dbgPost('first-delta', 'H6', {
            runId: __dbgRunId,
            type: part.type,
            sinceStartMs: now,
          })
        } else if (__dbgPrevDeltaTs !== undefined) {
          const dt = now - __dbgPrevDeltaTs
          __dbgInterDeltaSumMs += dt
          if (dt > __dbgInterDeltaMaxMs) __dbgInterDeltaMaxMs = dt
        }
        __dbgPrevDeltaTs = now
        __dbgDeltaCount += 1
        const text = (part as Record<string, unknown>).text
        if (typeof text === 'string') __dbgDeltaCharSum += text.length
      }
      if (part.type === 'finish-step') {
        const pm = (part as Record<string, unknown>).providerMetadata as
          | Record<string, unknown>
          | undefined
        const inner = (pm?.providerMetadata as Record<string, unknown>) ?? pm
        __dbgPost('finish-step providerMetadata', 'H1+H2+H3+H4', {
          providerMetadata: inner ?? null,
          deltaCount: __dbgDeltaCount,
          deltaCharSum: __dbgDeltaCharSum,
          firstDeltaTs: __dbgFirstDeltaTs,
          interDeltaSumMs: __dbgInterDeltaSumMs,
          interDeltaMaxMs: __dbgInterDeltaMaxMs,
          wallClockMs:
            __dbgFirstDeltaTs !== undefined ? now - __dbgFirstDeltaTs : null,
        })
      }
      if (part.type === 'finish') {
        ...
        const usage = fp.totalUsage ?? {}
        const wallMs =
          __dbgFirstDeltaTs !== undefined ? now - __dbgFirstDeltaTs : 0
        const clientTps =
          wallMs > 0 && typeof usage.outputTokens === 'number'
            ? (usage.outputTokens as number) / (wallMs / 1000)
            : 0
        __dbgPost('finish summary', 'H1+H2+H6', {
          usage,
          finishReason: fp.finishReason ?? null,
          wallClockMs: wallMs,
          clientFallbackTps: Math.round(clientTps * 10) / 10,
          serverTokensPerSecond:
            tokensPerSecond > 0 ? Math.round(tokensPerSecond * 10) / 10 : null,
          deltaCount: __dbgDeltaCount,
          deltaCharSum: __dbgDeltaCharSum,
          interDeltaSumMs: __dbgInterDeltaSumMs,
          interDeltaMaxMs: __dbgInterDeltaMaxMs,
          avgInterDeltaMs:
            __dbgDeltaCount > 1
              ? Math.round(
                  (__dbgInterDeltaSumMs / (__dbgDeltaCount - 1)) * 10
                ) / 10
              : null,
        })
      }
