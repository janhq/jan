...
describe('formatDate', () => {
  ...
  it('handles edge cases', () => {
    ...
    // Timezone may shift the rendered day for midnight UTC; assert stability rather than exact day.
    const oldDateResult = formatDate(oldDate)
    expect(oldDateResult).toMatch(/\d{4}/)
    expect(oldDateResult).toMatch(/(1899|1900)/)
