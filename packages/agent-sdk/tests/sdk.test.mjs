// The JavaScript SDK, driven end to end against a real runtime.
//
...
// network. Set `JAN_BIN` to the runtime binary to run it (the SDK's CI job
...
const missingRuntime = bin ? false : 'set JAN_BIN to the jan binary to run the SDK suite'
...
class Scratch {
  static async create(name) {
    const root = await mkdtemp(join(tmpdir(), `jan-sdk-${name}-`))
