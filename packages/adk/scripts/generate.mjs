#!/usr/bin/env node
//! Generate the ADK wire types from the committed RPC schema.
//!
...
//! other half of that guarantee: the JavaScript and Python ADKs derive their
//! request, event and enum types from the same document, and `--check` fails
//! when the committed output no longer matches it.
//!
//! One generator for both languages on purpose - the two ADKs are the same
...
//!
//! Usage:
//!   node packages/adk/scripts/generate.mjs
//!   node packages/adk/scripts/generate.mjs --schema <path> --ts <path> --py <path>
//!   node packages/adk/scripts/generate.mjs --check
...
const tsPath = resolve(flag('ts', join(repo, 'packages/adk/src/generated/rpc-types.d.ts')))
const jsPath = resolve(flag('js', join(repo, 'packages/adk/src/generated/rpc-types.js')))
const pyPath = resolve(flag('py', join(repo, 'adk/python/jan_adk/_generated.py')))
...
/// ADKs have, so a silent redefinition would mis-type a whole field.
const defs = new Map()
const gather = (node) => {
...
if (check) {
  ...
  if (stale.length) {
    console.error('generated ADK types are out of date with protocol/rpc-schema.json:')
    for (const path of stale) console.error(`  ${path}`)
    console.error('run: node packages/adk/scripts/generate.mjs')
    process.exit(1)
  }
  console.log(`generated ADK types match ${schemaPath}`)
} else {
