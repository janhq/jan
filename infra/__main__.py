...
platforms=[ docker_build.Platform.LINUX_AMD64, docker_build.Platform.LINUX_ARM64 ]
registries=[ docker_build.RegistryArgs(
    address=DOCKER_ADDR,
    password=config.require_secret("DOCKER_PAT"),
    username=DOCKER_USR,
)]
builder=docker_build.BuilderConfigArgs(name="cloud-nullstring-my-cool-builder")
...
container_env_minio=["MINIO_ROOT_USER=minioadmin","MINIO_ROOT_PASSWORD=minioadmin"]
...
container_minio = docker.Container(
    ...
    healthcheck=docker.ContainerHealthcheckArgs(
        ...
        tests=["CMD-SHELL", "wget --no-verbose --tries=1 --spider localhost:9000/minio/health/live || exit 1"],
        interval="30s",
        timeout="20s",
        retries=3,
    ),
