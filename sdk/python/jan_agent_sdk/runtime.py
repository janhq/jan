...
class JanRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        # `queue` here is the standard library's, not `._queue` - the SDK's own
        # event queue - which is why no module of this package shadows it.
        self._dispatch_queue: "queue.Queue[Optional[Any]]" = queue.Queue()
        self._dispatch_thread = threading.Thread(target=self._run_dispatch, name="jan-sdk-events", daemon=True)
        ...
        environment = {
            **os.environ,
            # The SDK owns this process and its lifetime; an update check nobody
    ...
    def _read_stdout(self) -> None:
        ...
        for line in self._process.stdout:
            ...
            try:
                ...
                self._fail(JanRuntimeError(f"the SDK could not route a frame: {error}", stderr=self.stderr))
                return
