...
/// Translate a `file://`-prefixed or bare path to an absolute `PathBuf`
/// without canonicalizing. Shared by `resolve_path` (which adds
/// canonicalization) and callers that must preserve symlinks (e.g. `mv`).
pub fn resolve_path_base<R: Runtime>(app_handle: tauri::AppHandle<R>, path: &str) -> PathBuf {
...
pub fn resolve_path<R: Runtime>(app_handle: tauri::AppHandle<R>, path: &str) -> PathBuf {
    let path = resolve_path_base(app_handle, path);
