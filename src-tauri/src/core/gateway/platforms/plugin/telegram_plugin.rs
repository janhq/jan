...
impl ChannelPlugin for TelegramPlugin {
    fn meta(&self) -> &ChannelMeta { &TELEGRAM_META }
    ...
    async fn validate_config(&self, account: &AccountConfig) -> PluginResult<()> {
        if account.settings.get("bot_token").is_none() {
            Err(PluginError::Configuration("Telegram requires bot_token".into()))
        } else { Ok(()) }
    }

    fn list_account_ids(&self, global_config: &Value) -> Vec<String> {
        global_config.get("telegram")
    ...
    async fn resolve_account(&self, global_config: &Value, id: &str) -> PluginResult<AccountConfig> {
        if let Some(arr) = global_config.get("telegram").and_then(|c| c.get("accounts")).and_then(|a| a.as_array()) {
            for account in arr {
    ...
    async fn start_account(&self, params: StartAccountParams) -> PluginResult<ChannelHandle> {
        ...
        Ok(ChannelHandle { platform: Platform::Telegram, account_id: params.account.id.clone(), state })
    }
    ...
    async fn parse_inbound(&self, payload: &Value) -> PluginResult<GatewayMessage> {
        crate::core::gateway::platforms::parse_telegram_payload(payload).map_err(|e| PluginError::Message(e))
    }

    async fn send_outbound(&self, _: &ChannelHandle, response: &GatewayResponse) -> PluginResult<()> {
        log::debug!("[Telegram Plugin] send_outbound to {}: {}", response.target_channel_id, &response.content[..response.content.len().min(100)]);
        Ok(())
    }

    fn format_outbound(&self, markdown: &str) -> String {
        crate::core::gateway::formatter::telegram::format_markdown(markdown)
    }

    fn chunk_limit(&self) -> usize { 4096 }
}
