...
impl PendingAuth {
    ...
    pub async fn complete(self, data_folder: &Path) -> Result<StoredCredentials, String> {
        ...
        // rmcp's token response carries its own extra-fields type; the `oauth2`
        // client returns the empty one, so restate the fields it did fill in.
        let mut tokens = OAuthTokenResponse::new(
            exchanged.access_token().clone(),
            exchanged.token_type().clone(),
            rmcp::transport::auth::VendorExtraTokenFields::default(),
        );
        tokens.set_expires_in(exchanged.expires_in().as_ref());
        tokens.set_refresh_token(exchanged.refresh_token().cloned());
...
pub async fn authorized_client(
    ...
    if let (Some(secret), OAuthState::Authorized(manager)) = (&stored.client_secret, &mut state) {
        manager
            .configure_client(
                OAuthClientConfig::new(stored.client_id.clone(), url.to_string())
                    .with_client_secret(secret.secret().clone()),
            )
...
mod tests {
    ...
    async fn registered_client_completes_pkce_and_retains_secret_for_refresh() {
        ...
        let client = authorized_client(
            dir.path(), "server", &url, &json!({}), reqwest13::Client::new(),
        ).await.unwrap().unwrap();
