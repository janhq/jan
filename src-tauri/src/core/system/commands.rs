...
#[serde(rename_all = "camelCase")]
pub struct CodexProviderProfileArtifactRequest {
    pub name: String,
    pub base_url: String,
    pub model: String,
...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct CodexProviderProfileArtifactResult {
    pub codex_home: String,
...
#[tauri::command]
pub fn write_codex_provider_profile_artifacts<R: Runtime>(
    app: AppHandle<R>,
    request: CodexProviderProfileArtifactRequest,
) -> Result<CodexProviderProfileArtifactResult, String> {
    let codex_home = request.codex_home.trim();
    if codex_home.is_empty() {
        return Err("CODEX_HOME is required".to_string());
    }
    ...
    fs::create_dir_all(&codex_home_path)
        .map_err(|error| format!("Failed to create CODEX_HOME: {error}"))?;

    let manifest_path = codex_home_path.join("desktop-provider-profile.json");
    let env_path = codex_home_path.join("desktop-provider.env");
    ...
    let manifest = serde_json::json!({
        "kind": "codex-desktop-provider-profile",
        "version": 1,
        "generatedAt": now_ms(),
        "profile": {
            "name": request.name,
            ...
            "codexHome": codex_home_path.display().to_string(),
        }
    });
    ...
    .map_err(|error| format!("Failed to write provider profile manifest: {error}"))?;

    let env_content = format!(
        "# Generated by Jan desktop runtime profile manager\nexport CODEX_HOME={}\nexport OPENAI_BASE_URL={}\nexport OPENAI_MODEL={}\nexport OPENAI_API_KEY=\"${{{}:-}}\"\n",
        shell_quote(&codex_home_path.display().to_string()),
        shell_quote(request.base_url.trim()),
        shell_quote(request.model.trim()),
        api_key_env
    );
    fs::write(&env_path, env_content)
        .map_err(|error| format!("Failed to write provider profile env: {error}"))?;

    Ok(CodexProviderProfileArtifactResult {
        codex_home: codex_home_path.display().to_string(),
...
#[tauri::command]
pub fn list_running_processes(limit: Option<usize>) -> Result<Vec<RunningProcessSnapshot>, String> {
        ...
        let processes = text
            ...
            .filter_map(|line| {
                let fields: Vec<String> = line
                    ...
                    .map(str::to_string)
                    .collect();
                let name = fields.get(0)?.trim().to_string();
                let pid = fields.get(1)?.trim().parse::<u32>().ok()?;
                Some(RunningProcessSnapshot {
                    pid,
                    name,
                    command: None,
                })
    ...
    #[cfg(not(windows))]
    {
        let output = Command::new("ps")
            ...
            .map_err(|error| format!("Failed to list processes: {error}"))?;
        if !output.status.success() {
            return Err("ps failed".to_string());
        }
        ...
        let processes = text
            ...
            .filter_map(|line| {
                ...
                Some(RunningProcessSnapshot { pid, name, command })
            })
            .take(limit)
            .collect();
        Ok(processes)
