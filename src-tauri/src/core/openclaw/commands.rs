...
async fn restart_gateway_cli() -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        // On Windows, service registration requires admin (schtasks) — skip it.
        // Spawn `openclaw gateway` as a child process directly.
        log::info!("Windows: restarting gateway as child process");
    ...
    #[cfg(not(target_os = "windows"))]
    {
        // On macOS/Linux, re-register the service then start it.
        let install_output = openclaw_command(&["gateway", "install"]).await
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .output()
            .await;
