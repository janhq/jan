...
fn select_program_interactively() -> String {
    ...
    let installed: Vec<bool> = CHOICES
        .iter()
        .map(|(key, _)| match *key {
            "codex" => is_command_installed("codex") || is_command_installed("npx"),
            "openclaw" => {
                is_command_installed("openclaw")
                    || is_command_installed("opencode")
                    || is_command_installed("npx")
            }
            key => is_command_installed(key),
        })
        .collect();
    ...
    if !installed.iter().any(|&i| i) {
        ...
        eprintln!("  (or ensure npm/npx is available for `npx @openai/codex` fallback)");
...
async fn handle_launch(
    ...
    let program_is_path = program_path.exists();
    let prog_name = program_path
        .file_name()
        .and_then(|n| n.to_str())
        .unwrap_or(&program)
        .to_ascii_lowercase();
    let is_claude = prog_name.contains("claude");
    let is_codex = prog_name.contains("codex");
    let is_openclaw = prog_name.contains("openclaw");

    if is_codex
        && !program_is_path
        && !is_command_installed("codex")
        && !is_command_installed("npx")
    {
        eprintln!("  ERROR: codex CLI not found in PATH.");
        eprintln!("  Install Codex CLI: npm install -g @openai/codex");
        eprintln!("  (or ensure npm/npx is available for `npx @openai/codex` fallback)");
        std::process::exit(1);
    }
    ...
    if is_openclaw
        ...
        && !is_command_installed("npx")
    {
        eprintln!("  ERROR: openclaw CLI not found in PATH.");
        eprintln!("  Install OpenClaw per its docs, or ensure npm/npx is available for `npx openclaw` fallback.");
        std::process::exit(1);
    }
    ...
    if is_openclaw {
        ...
        eprintln!("\n");
        eprintln!("  CODEX_HOME={}", codex_home.display());
    ...
    let (cmd_program, cmd_args) = if is_openclaw {
        ...
        if program_is_path || is_command_installed("codex") {
            (program.clone(), program_args.clone())
        } else {
            using_codex_npx = true;
            let mut args = vec!["-y".to_string(), "@openai/codex".to_string()];
            args.extend(program_args.clone());
            ("npx".to_string(), args)
        }
    } else {
    ...
    if using_codex_npx {
        eprintln!("  INFO: 'codex' not found, using fallback `npx @openai/codex`.");
    }

    let launch_cmd = if cmd_args.is_empty() {
        cmd_program.clone()
    } else {
        format!("{} {}", cmd_program, cmd_args.join(" "))
    };
