    ...
    match cmd {
        ...
        ModelscopeCommands::Full { model_id, token } => {
            ...
            let detail = match get_modelscope_model_detail(owner, repo_name, token).await {
                Ok(d) => {
                    eprintln!(
                        "  ✓ Detail fetched: {} (downloads: {}, likes: {})",
                        d.model.base.id, d.model.base.downloads, d.model.base.likes
                    );
                    d
                }
                Err(e) => {
...
fn select_best_gguf(
    files: &app_lib::core::modelscope::models::ModelScopeFileListResult,
) -> Option<app_lib::core::modelscope::models::ModelScopeFile> {
    let ggufs: Vec<_> = files
        .Files
        .iter()
        .filter(|f| {
    ...
    let best = ggufs.iter().max_by_key(|f| priority(&f.Name)).copied()?;
