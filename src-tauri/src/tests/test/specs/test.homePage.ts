...
describe('Verify Jan Homepage', () => {
  ...
  it('user should be able to open new chat', async () => {
    ...
    const result = await homePage.elementShouldBeVisible(homePage.elements.newChatButton);
  ...
  it('user should be able to open assistants', async () => {
    ...
    const result = await homePage.elementShouldBeVisible(homePage.elements.assistantsButton);
    expect(result).toBeTruthy();
  });
