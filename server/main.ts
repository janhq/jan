  ...
  (req: Request, res: Response, next: NextFunction): void => {
    ...
    switch (method) {
      case 'deleteFile':
        ...
        break
      case 'downloadFile':
        downloadFile(args.downloadUrl, args.fileName)
          .then(() => res.json(Object()))
          .catch((err: any) => next(err))
        break
      default:
...
async function invokeFunction(
  ...
  const module = require(
    /* webpackIgnore: true */ path.join(dataDir, '', modulePath)
  )
...
function downloadingFile(): ProgressState | undefined {
  const obj = Object.values(downloadProgress).find(
    (obj) => obj && typeof obj.success === 'undefined'
  )
  return obj
}
