...
export class ChatPage extends BasePage {
  public elements: ChatElements
  constructor() {
    super()
    this.elements = {
      chatInput: `//textarea[@placeholder='Ask me anything...']`,
      sendButton: `//button/*[@fill="none"]`,
  ...
  async sendMessage(message: string): Promise<void> {
    ...
    await this.click(this.elements.sendButton)
  }

  async verifyChatInputVisible(): Promise<void> {
    const result = await this.isDisplayed(this.elements.chatInput)
    if (!result) {
  ...
  async selectModel(model: string): Promise<void> {
    if (!(await this.isTextContains(model))) {
      await this.click(this.elements.modelBtn)
      if (await this.isText(model)) {
        const locator = String.format(this.elements.modelItem, model)
        await this.click(locator)
        if (await this.isDisplayed(this.elements.searchModelInput)) {
  ...
  async tapModelSetting(): Promise<void> {
    await this.click(this.elements.modelSettingBtn)
  }

  async isTable(): Promise<boolean> {
    return this.isDisplayed(this.elements.tableRes)
  }
  ...
  async tapThought(index: number = 1): Promise<void> {
    ...
    await this.click(thought)
  }

  async isThought(index: number = 1): Promise<boolean> {
    const thought = this.elements.thoughtBtn + '[' + index + ']'
    return await this.isDisplayed(thought)
  }
  ...
  async editMsgSend(message: string): Promise<void> {
    ...
    await this.isDisplayed(this.elements.saveBtn)
  }
  ...
  async deleteAllHistory(): Promise<void> {
    ...
    if (history.length > 0) {
      ...
      await this.click(this.elements.deleteBtn)
    }
  }
  ...
  async renameHistory(name: string): Promise<void> {
    ...
    await this.click(this.elements.renameBtn)
  }
