...
Jan is an MCP host that allws you to download different clients and servers and use them to accomplish a task.
...
Before we begin, you will need to enable Jan's MCP functionalitie at `Settings` > `MCP Servers`, and toggle
...
the MCP ecosystem grows, sowill the potential applications within Jan.
