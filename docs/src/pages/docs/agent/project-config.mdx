...
description: JAN.md and agent.toml - per-project instructions, model, budget, and tool policy.
keywords: [Jan, Jan Agent, agent.toml, JAN.md, project config, instructions, budget]
---
...
project directory itself holds only `JAN.md`.

```
your-project/
`-- JAN.md                 # instructions for the agent
...
<Callout type="tip">
Commit `JAN.md` so the whole team gets the same instructions. It is the only file Jan keeps in the
...
Jan reads `JAN.md` from the project root and each of its ancestors; the nearest file wins. Write one
by hand or with `/init`.
...
[shell confinement](/docs/agent/permissions#the-shell-sandbox) for every session in it; it is off
