...
keywords: [Jan, Jan Agent, ADK, host tools, function calling, JavaScript, Python]
---
...
The model chooses a tool and supplies arguments. The ADK calls your handler. Your handler's result
...
First complete the [JavaScript](/docs/agent/adk-ts) or [Python](/docs/agent/adk-python) quickstart.
These examples use the same installed ADK and provider configuration.
...
It asks to call the tool; the ADK runs the handler in your process. No manual JSON messages are needed.
...
- **Jan owns approval by default.** Your app must handle `permission_request` events and answer
  them. Otherwise the turn waits. See [permission prompts](/docs/agent/adk-runtime#answer-permission-prompts).
...
[Process & failures](/docs/agent/adk-runtime) covers stopping turns, permissions, shutdown and
