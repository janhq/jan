...
keywords: [Jan, Jan Agent, ADK, process, permissions, errors, cancellation]
---
...
[JavaScript](/docs/agent/adk-ts) and [Python](/docs/agent/adk-python) quickstarts show both patterns.

The ADK closes the channel and reaps its child process, escalating when the process does not exit.
...
```
  ...
  <Tabs.Tab>
```python
from jan_adk import install_runtime, JanRuntime
