...
| Google Gemini | Nested in the call's `functionResponse.parts` as `inlineData` |
| OpenAI Responses | In the `function_call_output`'s item list, as `input_image` |

Gemini accepts only inline (`data:`) images; a remote image URL is dropped. A part that is not an
image is dropped on every provider rather than passed as a file. An
