...
(or `(image)` for an image alone). Today the provider request carries the text of the parts only:
images in tool messages are not yet forwarded to the model. An optional `details` object is for your
