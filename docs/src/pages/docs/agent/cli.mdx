...
```bash
jan cli models refresh                    # every reachable provider
...
outage than a retirement. The command exits non-zero if any provider could not be listed, so a
partial refresh is never reported as a complete one.

The interactive console does the same thing: opening `/model` re-lists each provider once per
session, and <kbd>Ctrl</kbd>+<kbd>R</kbd> inside the picker re-lists on demand.
