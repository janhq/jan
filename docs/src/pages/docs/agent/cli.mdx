...
(or `(image)` for an image alone). On the provider request the tool message is the parts' text,
and the images follow the run of tool messages as one user message, each labeled with the call it
answers: most providers accept only text in a tool message. An optional `details` object is for your
own display: it is **never sent to the model**, and the run echoes it as a record right after that
call's `tool_result`:
