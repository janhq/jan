...
Run to completion, or until the session token budget is spent. There is no turn cap by default - the
agent runs as many turns as the task needs, bounded by `[budget].max_tokens`, by `--max-turns` /
`--max-session-tokens`, or by cancelling it.
...
| `--worktree` | Work in a dedicated git worktree instead of the project directory. Off by default |
| `--no-worktree` | Work in the project directory, overriding a persistent `worktree` setting |
| `--max-turns <N>` | Stop after at most N agentic turns. `0` means unbounded, which is the default |
| `--max-session-tokens <N>` | Session token ceiling for this run, overriding `[budget].max_tokens`. `0` means unbounded |
