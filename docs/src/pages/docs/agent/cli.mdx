...
| `request_sha256` | SHA-256 of the request body as Jan built it, as canonical JSON (object keys sorted): the whole request's identity, so two runs are comparable even when something this record does not itemize changed |
| `body_bytes` | The built body's serialized length, measured before the provider adapter serializes it its own way |
| `tools_sha256` | SHA-256 of the `tools` array exactly as sent. Absent when the request carries no non-empty tool list. The tool array can change while the model id does not, which is why it is hashed separately from the body |
...
Emitted for subagent turns as well as the main run's, and for the run's side calls (a compaction
summary, a `/goal` evaluation), since those are real requests with real spend. It is a record on
the event stream, so it also reaches RPC clients as `item/request_provenance`.
...
built, before the provider's adapter serializes it into that provider's own shape, so it is stable
for a given run and comparable across runs against the same provider. Two runs on different wire
