...
description: Install the preview ADK, connect a model, and stream your first Jan Agent response from Python.
keywords: [Jan, Jan Agent, ADK, Python, quickstart, embedding]
---
...
SHA-256 the build published. The [download page](https://delta.jan.ai/adk-nightly/index.html) shows
...
blocking ADK work in a worker thread rather than on your event loop.
...
| The wheel download fails | No nightly is available right now. Use the [source install](/docs/agent/adk-nightly#source-install-fallback). |
