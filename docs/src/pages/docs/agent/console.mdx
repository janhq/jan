...
While a turn runs, the input row shows a Braille throbber and an action word that turns over every
few seconds, so a long turn does not read as a hung UI.

Pick a glyph to sweep back and forth along that word instead from `/settings > wave`, or set it in
`~/.jan/config.toml` directly:

```toml
wave = "👋"
```

Any string works - `"🍌"`, `"~"`, `"<o>"`. The glyph replaces the throbber rather than joining it,
and covers the characters it passes over instead of pushing them along, so the row never changes
width no matter how wide the glyph is.

Unlike most settings, this one applies the moment you save it - clearing the field in `/settings`
(or pressing <kbd>x</kbd> on the row) puts the throbber back with no restart.

Unset by default: a terminal without the font for your glyph draws it as a missing-character box,
so this is opt-in rather than something a fresh install inherits.
