...
- Use `ephemeral: true` / `ephemeral=True` when the conversation should not be saved. An
  ephemeral session also neither reads nor writes project memory, so one session's answers never
  reach a later session in the same `cwd`.
