...
- Use `ephemeral: true` / `ephemeral=True` when the conversation should not be saved. Its answers
  are also kept out of automatic memory recall, so one session's answers never reach a later
  session in the same `cwd`. [Memory notes](/docs/agent/memory) written with `memory_write` in
  other sessions are still listed in its prompt.
