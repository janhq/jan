...
**You do not need this page for the SDK quickstarts.** The SDK handles the messages for you.
...
| Best for | SDKs and long-lived applications | Direct CLI automation |
...
[JavaScript](/docs/agent/sdk-ts) or [Python](/docs/agent/sdk-python) SDK.
...
<Callout type="info">
The current SDKs use RPC. If your binary has no `rpc` command, update the nightly runtime.
...
Prefer the SDK unless you need to own the pipes yourself. The runnable examples in
[`examples/agent-sdk`](https://github.com/janhq/jan/tree/main/examples/agent-sdk) show that
lower-level alternative. CI runs all four against a real runtime and checks these excerpts
against their source. They are not the RPC SDK examples above.

<details>
<summary>Read a streamed answer without the SDK</summary>
...
<details>
<summary>Answer a host tool call without the SDK</summary>
...
the request id. The SDK performs that dispatch for you.
