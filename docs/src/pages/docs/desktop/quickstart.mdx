...
import { FolderOpen, Bot, Cpu, Plug, Brain, Hammer } from 'lucide-react'
...
<Callout type="tip">
Jan has built-in web search and fetch tools, so you can ask about the latest news and current events without any extra setup. If you'd rather use a different provider, you can also configure the [Exa](https://exa.ai) MCP server under **Settings > MCP Servers** with your own API key.
</Callout>
...
<DocCards>
  <DocCard title="Cowork (Preview)" href="/docs/desktop/cowork" icon={<Hammer size={20} />}>Hand Jan a task it works through on its own — in a sandbox or directly on your project.</DocCard>
