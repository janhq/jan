...
AMD GPUs can use either **ROCm/HIP** (better performance) or **Vulkan** (no extra setup).

**Option A — ROCm/HIP (recommended for supported GPUs):**

<Steps>

### Step 1: Install ROCm

Follow the [official AMD ROCm installation guide](https://rocm.docs.amd.com/projects/install-on-linux/en/latest/).
...
### Step 2: Enable HIP Backend

1. Navigate to **Settings** > **Local Engine** > **Llama.cpp**
2. Select the `linux-hip-x64` backend

</Steps>

**Option B — Vulkan (no extra setup):**

1. Navigate to **Settings** > **Local Engine** > **Llama.cpp**
2. Select a Vulkan backend (`linux-vulkan-common_cpus-x64`). Details in our [llama.cpp guide](/docs/desktop/local-engine/llama-cpp).

<Callout type="info">
If ROCm is installed, Jan will automatically prefer HIP over Vulkan for better performance.
</Callout>
