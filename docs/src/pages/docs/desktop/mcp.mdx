...
<Callout type="info">
  ...
  - For cloud models (like Claude or GPT-4): Choose a model in Jan that includes **tools** capability. For built-in providers like Anthropic, Jan reads the provider's model capabilities automatically after you add your API key.
...
focuses on user-managed permissions to mitigate this risk — you must approve each tool call individually. Approval
panels appear **inline inside the tool card** in the chat, showing the exact arguments the model wants to pass
before you accept or deny. You can also enable **Allow All MCP Tool Permission** in Settings to pre-approve all
tools (**Allow All MCP Tool Permissions** in Settings → MCP Servers).
...
Before we begin, go to `Settings` > `MCP Servers` and toggle the `Allow All MCP Tool Permissions` switch ON if you want to pre-approve tool calls.
...
- Go back to Jan and pick a model with strong tool-use capabilities, such as a Claude Sonnet or Opus model. After you enter your API key in **Model Providers > Anthropic**, Jan will list the available Anthropic models automatically. Choose one that shows **tools** capability in the model picker. No separate Anthropic tool toggle is required.
...
local script to analyze data and then summarize the findings, all orchestrated through Jan's interface. As
