...
version: 0.8.4
description: "Jan v0.8.4 adds native web_search / web_fetch tools, per-model chat-template kwargs for llama.cpp, a token counter for remote and MLX providers, and moves settings and credentials off browser localStorage into a backend store - alongside a batch of chat, MCP, provider, and streaming fixes."
date: 2026-07-21
---
...
Provider settings and credentials previously persisted in the webview's `localStorage`. In 0.8.4 they move to a backend-managed store. Your existing settings and API keys are migrated automatically on first launch - no manual action is needed. If you run multiple Jan installations or restore from an old profile, expect settings to be read from the new backend store rather than localStorage from this version on.
