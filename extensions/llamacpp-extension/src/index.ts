...
<<<<<<< HEAD
    ...
    try {
      ...
      // Check for manual capability override in config
      if (modelConfig.capabilities?.includes('tools')) {
        logger.info(`Tool support enabled via config for ${modelId}`)
        return true
      }
      ...
      // Multi-pattern detection for various tool calling syntaxes
      const toolPatterns = [
        /\btools?\b/i, // Original: 'tools' or 'tool'
        /<\|tool(?:_call|_use)?\|>/i, // <|tool|>, <|tool_call|>, <|tool_use|>
        /\bfunction(?:_call|s)?\b/i, // 'function', 'function_call', 'functions'
        /<\|(?:begin_|end_)?(?:tool|function)\|>/i, // Custom markers
        /\{\s*"?(?:tool|function)s?"?\s*:/i, // JSON-style: {"tools":...}
        /<tool>/i, // XML-style: <tool>
        /\[TOOL\]/i, // Bracket notation: [TOOL]
        /###\s*Tools?:/i, // Markdown-style: ### Tools:
      ]

      const hasToolSupport = toolPatterns.some((pattern) =>
        pattern.test(chatTemplate)
      )

      if (hasToolSupport) {
        logger.info(`Tool support detected in chat_template for ${modelId}`)
        return true
      }

      // Fallback: Model name heuristics for known tool-capable models
      const toolCapableModels = [
        /llama-?3\.?[12]/i, // Llama 3.1, 3.2
        /mistral.*(?:instruct|v0\.[23])/i, // Mistral Instruct variants
        /yi.*chat/i, // Yi Chat models
        /deepseek.*(?:coder|chat)/i, // DeepSeek Coder/Chat
        /qwen.*chat/i, // Qwen Chat models
        /command-?r\+?/i, // Cohere Command-R
        /functionary/i, // Functionary models
        /gorilla/i, // Gorilla models
        /jan-nano-(?:32k|128k)/i, // Jan Nano models
      ]

      const modelNameIndicatesTool = toolCapableModels.some((pattern) =>
        pattern.test(modelId.toLowerCase())
      )

      if (modelNameIndicatesTool) {
        logger.info(
          `Tool support inferred from model name for ${modelId} (may need verification)`
        )
        return true
      }
