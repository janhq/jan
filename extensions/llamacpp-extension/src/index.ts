  ...
  private async performLoad(
    ...
    if (draftModelId) {
      try {
        ...
        const draftConfig = await invoke<ModelConfig>('read_yaml', {
          path: draftConfigPath,
        })
        cfg.draft_model_path = await joinPath([
          janDataFolderPath,
          draftConfig.model_path,
        ])
        logger.info(`Resolved draft model path: ${cfg.draft_model_path}`)
      } catch (e) {
