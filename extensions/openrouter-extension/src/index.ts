 ...
 * @version 1.0.0
 * @module openrouter-extension/src/index
 */
 ...
 * It also subscribes to events emitted by the @janhq/core package and handles new message requests.
 */
export default class OpenRouterProvider extends RemoteOAIEngine {
  inferenceUrl: string = ''
  baseURL: string = ''
  ...
  override async onLoad(): Promise<void> {
    ...
    const models = DEFAULT_MODELS.map((model) => {
      // we can check tools support with
      // https://openrouter.ai/api/v1/models/qwen/qwen3-30b-a3b%3Afree/endpoints
      // however, even though some providers report tools is supported, it does
      // not work correctly.
      return {
        sources: [],
        object: 'model',
